...
int main(int argc, char *argv[])
{
    ...
    if (tomar_muestra(pid, &anterior) == -1) {
        fprintf(stderr, "No se pudo tomar la primera muestra\n");
        return 1;
    }
    ...
    if (tomar_muestra(pid, &actual) == -1) {
        fprintf(stderr, "No se pudo tomar la segunda muestra\n");
        return 1;
    }

    double dif_tiempo =
        calcular_dif_tiempo(tiempo_anterior, tiempo_actual);

    double cpu =
        calcular_cpu(anterior.cpu_ticks, actual.cpu_ticks, dif_tiempo);
